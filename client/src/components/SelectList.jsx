...
const SelectList = ({ lists, selected, setSelected, label }) => {
    const getSubjectName = (subjectId) => {
        ...
        const foundSubject = lists?.find((subject) => subject.id === subjectId);
    ...
    return (
        <div className='w-full'>
            {label && <p className='text-slate-800'>{label}</p>}
            ...
            <Listbox value={selected} onChange={setSelected}>
                <div className='relative mt-1'>
                    <Listbox.Button className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
                        <span className='block truncate'>{selected?.length === 0 ? ("Select") : getSubjectName(selected) ? getSubjectName(selected) : "Subject Not Found" }</span>
